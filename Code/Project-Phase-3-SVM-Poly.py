...
dataFrame = dataFrame.sample(n=60000).reset_index(drop=True)
...
print("\nThe best parameter for SVM (polynomial kernel) is: ", grid_search.best_params_)
best_model = grid_search.best_estimator_

y_pred = best_model.predict(X_test)
print("\nThe test accuracy of SVM (polynomial kernel) is: ", accuracy_score(y_test, y_pred))
print("\nThe confusion matrix of SVM (polynomial kernel) is: \n", confusion_matrix(y_test,y_pred))
print("\nThe precision of SVM (polynomial kernel) is: ", precision_score(y_test, y_pred))
print("\nThe recall of SVM (polynomial kernel) is: ", recall_score(y_test,y_pred))

tn, fp, fn, tp = confusion_matrix(y_test, y_pred).ravel()
specificity = tn / (tn + fp)

print("\nThe specificity of SVM (polynomial kernel) is: ", specificity)
print("\nThe f1 score of SVM (polynomial kernel) is: ", f1_score(y_test, y_pred))
...
plt.plot(fpr, tpr, linestyle='--', label='SVM (polynomial kernel) AUC = %0.2f' % roc_auc_score(y_test, y_pred))
...
joblib.dump(grid_search, 'svm_poly.pkl')