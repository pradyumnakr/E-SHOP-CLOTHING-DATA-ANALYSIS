...
print("\nThe best parameter for SVM (rbf kernel) is: ", grid_search.best_params_)
best_model = grid_search.best_estimator_

y_pred = best_model.predict(X_test)
print("\nThe test accuracy of SVM (rbf kernel) is: ", accuracy_score(y_test, y_pred))
print("\nThe confusion matrix of SVM (rbf kernel) is: \n", confusion_matrix(y_test,y_pred))
print("\nThe precision of SVM (rbf kernel) is: ", precision_score(y_test, y_pred))
print("\nThe recall of SVM (rbf kernel) is: ", recall_score(y_test,y_pred))

tn, fp, fn, tp = confusion_matrix(y_test, y_pred).ravel()
specificity = tn / (tn + fp)

print("\nThe specificity of SVM (rbf kernel) is: ", specificity)
print("\nThe f1 score of SVM (rbf kernel) is: ", f1_score(y_test, y_pred))
...
plt.plot(fpr, tpr, linestyle='--', label='SVM (rbf kernel) AUC = %0.2f' % roc_auc_score(y_test, y_pred))
