...
print("\nThe best parameter for Random Forest (Stacking) is: ", grid_search.best_params_)
best_model = grid_search.best_estimator_

y_pred = best_model.predict(X_test)
print("\nThe test accuracy of Random Forest (Stacking) is: ", accuracy_score(y_test, y_pred))
print("\nThe confusion matrix of Random Forest (Stacking) is: \n", confusion_matrix(y_test,y_pred))
print("\nThe precision of Random Forest (Stacking) is: ", precision_score(y_test, y_pred))
print("\nThe recall of Random Forest (Stacking) is: ", recall_score(y_test,y_pred))

tn, fp, fn, tp = confusion_matrix(y_test, y_pred).ravel()
specificity = tn / (tn + fp)

print("\nThe specificity of Random Forest (Stacking) is: ", specificity)
print("\nThe f1 score of Random Forest (Stacking) is: ", f1_score(y_test, y_pred))
...
plt.plot(fpr, tpr, linestyle='--', label='Random Forest (Stacking) AUC = %0.2f' % roc_auc_score(y_test, y_pred))
...
joblib.dump(grid_search, 'rf_stacking.pkl')